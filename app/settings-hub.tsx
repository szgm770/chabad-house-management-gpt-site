...
export default function SettingsHub({initial="general"}:{initial?:Topic}){
 ...
 return <div className={`settings-hub settings-clean ${mobileOpen?"mobile-topic-open":""}`}>
  ...
  <div className="settings-layout"><nav className="settings-topics" style={{alignSelf:"start"}} aria-label="נושאי הגדרות">{topics.map(t=><button key={t.id} aria-current={topic===t.id?"page":undefined} className={topic===t.id?"active":""} onClick={()=>choose(t.id)}><t.icon/><span><b>{t.label}</b><small>{t.detail}</small></span><ChevronLeft className="topic-chevron"/></button>)}</nav>
