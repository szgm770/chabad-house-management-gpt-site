...
export async function POST(request: Request) {
  try {
    ...
    if (!body.KevaId || !body.ClientName || !body.Amount || !body.NextDate) return Response.json({ error: "חסרים שדות חובה של הוראת קבע" }, { status: 400 });
    const db = getDb();
    ...
    if (existing.length) return Response.json({ ok: true, duplicate: true, recurring: existing[0] });
    ...
    const start = body.NextDate.slice(0, 10), end = months ? (() => { const date = new Date(`${start}T12:00:00Z`); date.setUTCMonth(date.getUTCMonth() + months); return date.toISOString().slice(0, 10); })() : null;
    const [recurring] = await db.insert(recurringCommitments).values({ donorCardId, personId: linkedPeople.length === 1 ? linkedPeople[0].id : null, amount: Number(body.Amount), currency: body.Currency === "2" ? "USD" : "ILS", paymentMethod: "אשראי", startDate: start, durationMonths: months, expectedDay: Number(start.slice(8, 10)), endDate: end, status: "active", source: "nedarim-plus", externalId: body.KevaId, rawPayload: JSON.stringify(Object.fromEntries(Object.entries(body).filter(([key]) => !["Tokef"].includes(key)))) }).returning();
    await db.update(donorCards).set({ recurringStatus: "ACTIVE", recurringAmount: Number(body.Amount), recurringDay: Number(start.slice(8, 10)), recurringStartDate: start, recurringEndDate: end }).where(eq(donorCards.id, donorCardId));
    return Response.json({ ok: true, recurring }, { status: 201 });
