...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const session = await createSessionToken({ email: profile.email, name: profile.name || null, exp: Date.now() + sessionMaxAge * 1000 });
    ...
    response.cookies.set("chabad_session", session, { httpOnly: true, secure: true, sameSite: "lax", maxAge: sessionMaxAge, path: "/" });
