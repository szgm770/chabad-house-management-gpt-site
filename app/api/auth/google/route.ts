...
export async function GET(request: NextRequest) {
  ...
  const state = crypto.randomUUID(), returnTo = request.nextUrl.searchParams.get("return_to") || "/", origin = process.env.APP_URL || request.nextUrl.origin, jar = await cookies();
  jar.set("oauth_state", state, { httpOnly: true, secure: true, sameSite: "lax", maxAge: 600, path: "/" });
  jar.set("oauth_return", returnTo.startsWith("/") && !returnTo.startsWith("//") ? returnTo : "/", { httpOnly: true, secure: true, sameSite: "lax", maxAge: 600, path: "/" });
