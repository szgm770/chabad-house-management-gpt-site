...
export async function GET(request:Request){
 try{
  ...
  let items=cards.map(card=>{
   const persons=peopleByCard.get(card.id)||[],stats=statsByCard.get(card.id),lastDonationDate=stats?.lastDonationDate?normalizeAppDate(stats.lastDonationDate):"",recurring=recurringByCard.get(card.id)||[],campaigns=campaignsByCard.get(card.id)||[],manual=parseGroups(card.manualRelationshipGroups),automatic:string[]=[];
   ...
   if(campaigns.length)automatic.push("active_campaign");if(active)automatic.push("recurring_active");if(active&&card.eligibleForRecurringIncrease)automatic.push("recurring_increase");if(ended||card.recurringStatus==="ENDED")automatic.push("recurring_ended");if(lastDonationDate)automatic.push("recent_donor");
   const relationshipGroups=[...new Set([...automatic,...manual])],ranks=relationshipGroups.map(key=>configMap.get(key)?.rank).filter((rank):rank is number=>typeof rank==="number"),priorityRank=ranks.length?Math.min(...ranks):99,due=card.nextContactDate&&card.nextContactDate<=new Date().toISOString().slice(0,10),lastTime=lastDonationDate?new Date(`${lastDonationDate}T12:00:00`).getTime():0,recency=lastTime?Math.max(0,365-Math.floor((Date.now()-lastTime)/86400000)):0,priorityScore=(100-priorityRank)*10000+(due?5000:0)+Math.min(4000,Math.round(Number(stats?.periodTotal||0)))+recency;
   return {...card,people:persons,phone:card.phone||persons.find(p=>p.phone)?.phone||"",relationshipGroups,manualRelationshipGroups:manual,automaticRelationshipGroups:automatic,priorityRank,priorityScore,lastDonationDate:lastDonationDate||null,periodTotal:Number(stats?.periodTotal||0),allTotal:Number(stats?.allTotal||0),recurringStatus:active?"active":ended?"ended":card.recurringStatus.toLowerCase(),recurringAmount:Number(active?.amount||card.recurringAmount||0),campaigns};
  });
