...
import { Check, Pencil, Plus, Save, ShieldAlert, Trash2, UserRound, UsersRound, X } from "lucide-react";
...
export default function UserPermissionsSettings(){
 ...
 function saveUser(){const name=form.name.trim(),email=form.email.trim().toLowerCase();if(!name||!/^\S+@\S+\.\S+$/.test(email)){setNotice("יש להזין שם וכתובת דוא״ל תקינה.");return}if(users.some(user=>user.email.toLowerCase()===email&&user.id!==form.id)){setNotice("כתובת הדוא״ל כבר נמצאת ברשימת המשתמשים.");return}const user={...form,id:form.id||crypto.randomUUID(),name,email},next=form.id?users.map(item=>item.id===form.id?user:item):[...users,user];setUsers(next);setEditing(false);void persist(next,permissions)}
 ...
 return <div className="user-access-settings">{notice&&<div className="save-notice"><Check/>{notice}</div>}
  ...
  {editing&&<div className="access-editor" role="dialog" aria-modal="true" aria-labelledby="access-editor-title"><button className="access-editor-backdrop" aria-label="סגירת החלון" onClick={()=>setEditing(false)}/><div className="access-editor-card"><div className="access-editor-head"><div><h2 id="access-editor-title">{form.id?"עריכת משתמש":"הוספת משתמש"}</h2><p>המשתמש יישמר ברשימה בלבד; לא תישלח הזמנה בשלב הזה.</p></div><button aria-label="סגירה" onClick={()=>setEditing(false)}><X/></button></div><div className="access-editor-fields"><label><span>שם מלא *</span><input autoFocus value={form.name} onChange={e=>setForm({...form,name:e.target.value})}/></label><label><span>כתובת דוא״ל *</span><input type="email" dir="ltr" value={form.email} onChange={e=>setForm({...form,email:e.target.value})}/></label><label><span>תפקיד</span><select value={form.role} onChange={e=>setForm({...form,role:e.target.value as RoleKey})}>{roles.map(role=><option key={role.key} value={role.key}>{role.label}</option>)}</select></label><label><span>מצב</span><select value={form.status} onChange={e=>setForm({...form,status:e.target.value as UserStatus})}><option value="active">פעיל</option><option value="inactive">מושבת</option></select></label></div><div className="dialog-actions"><Button variant="outline" onClick={()=>setEditing(false)}>ביטול</Button><Button onClick={saveUser}>שמירת משתמש</Button></div></div></div>}
 </div>
}
