...
export default function UserPermissionsSettings(){
 ...
 async function saveUser(){const name=form.name.trim(),email=form.email.trim().toLowerCase();if(!name||!/^\S+@\S+\.\S+$/.test(email)){setNotice("יש להזין שם וכתובת דוא״ל תקינה.");return}if(users.some(user=>user.email.toLowerCase()===email&&user.id!==form.id)){setNotice("כתובת הדוא״ל כבר נמצאת ברשימת המשתמשים.");return}const user={...form,id:form.id||crypto.randomUUID(),name,email};const next=form.id?users.map(item=>item.id===form.id?user:item):[...users,user];if(await persist(next,permissions))setEditing(false)}
