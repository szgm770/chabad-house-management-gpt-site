...
export default function MovementImportExport({onImported}:{onImported:()=>void}){
  ...
  async function importFile(event:ChangeEvent<HTMLInputElement>){
    ...
    const rows=data.map(row=>({movementType:get(row,"סוג תנועה")==="הוצאה"?"expense" as const:"donation" as const,donorId:Number(get(row,"מזהה כרטיס תורם"))||undefined,personId:Number(get(row,"מזהה אדם"))||undefined,donorName:get(row,"שם התורם / עבור מה ההוצאה"),idNumber:get(row,"תעודת זהות"),phone:get(row,"טלפון"),email:get(row,"דוא״ל"),address:get(row,"כתובת"),amount:get(row,"סכום"),currency:get(row,"מטבע")||"ILS",date:get(row,"תאריך לועזי"),paymentMethod:get(row,"אמצעי תשלום"),department:get(row,"מחלקה ראשית"),subcategory:get(row,"קטגוריית משנה"),purpose:get(row,"קטגוריית משנה"),isRecurring:["כן","true","1"].includes(get(row,"תנועה קבועה").toLowerCase()),reason:get(row,"הערות"),externalId:get(row,"מזהה חיצוני")||undefined})).filter(row=>row.donorName&&row.amount&&row.date);
