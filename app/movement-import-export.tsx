  ...
  async function confirmImport(){
    ...
    try{
      for(let chunkIndex=0;chunkIndex<totalChunks;chunkIndex++){
        const rows=pending.rows.slice(chunkIndex*chunkSize,(chunkIndex+1)*chunkSize);
        const response=await fetch("/api/movements/import",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({filename:pending.filename,fingerprint:pending.fingerprint,rows,chunkIndex,totalChunks,totalRows:pending.rows.length})});
        ...
        for(const item of result.errors||[])errors.set(item.reason,(errors.get(item.reason)||0)+Number(item.count||0));
        const processed=Math.min((chunkIndex+1)*chunkSize,pending.rows.length);
        setProgress({processed,total:pending.rows.length,imported:totals.imported,skipped:totals.skipped+totals.duplicates});
      }
      const errorText=errors.size?` סיבות הדילוג: ${[...errors].map(([reason,count])=>`${reason} (${count})`).join(", ")}.`:"";
      setMessage(`הייבוא הושלם: ${totals.imported} תנועות נוספו. ${totals.matched} שויכו לכרטיס קיים, ${totals.created} יצרו כרטיס חדש${totals.review?`, ${totals.review} הועברו לבדיקת התאמה`:""}${totals.skipped+totals.duplicates?`, ${totals.skipped+totals.duplicates} שורות דולגו`:""}.${errorText}`);setPending(null);onImported();
    }catch(error){setMessage(`${error instanceof Error?error.message:"הייבוא נכשל"}. אפשר לנסות שוב; ניסיון חוזר מתחיל את הקובץ בצורה נקייה.`)}finally{setBusy(false)}
  }
  return <><Button variant="outline" onClick={()=>setOpen(true)}><FileSpreadsheet/>ייבוא / ייצוא</Button><Dialog open={open} onOpenChange={value=>{if(!busy)setOpen(value)}}><DialogContent dir="rtl" className="import-dialog"><DialogHeader className="text-right"><DialogTitle>ייבוא וייצוא תנועות</DialogTitle><DialogDescription>תהליך בטוח ומסודר להעברת הכנסות והוצאות באמצעות Excel.</DialogDescription></DialogHeader><div className="import-steps"><article><span>1</span><div><b>הורדת קובץ לדוגמה</b><p>התבנית כוללת מזהי כרטיס ואדם, ת״ז, טלפון, דוא״ל וכתובת להתאמה מדויקת.</p></div><Button variant="outline" onClick={template}><Download/>הורדת התבנית</Button></article><article><span>2</span><div><b>העלאה ובדיקה</b><p>גם שורות חלקיות ייקלטו; רק סכום תקין הוא חובה.</p></div><Button onClick={()=>fileRef.current?.click()}><Upload/>בחירת קובץ</Button><input ref={fileRef} type="file" accept=".csv,text/csv" onChange={e=>void importFile(e)}/></article><article><span>3</span><div><b>ייצוא כל התנועות</b><p>הורדת כל ההכנסות וההוצאות עם פרטי הזיהוי הקיימים.</p></div><Button variant="outline" onClick={()=>void exportAll()}><Download/>ייצוא הרשימה</Button></article></div>{message&&<p className="import-message" role="status">{message}</p>}{pending&&<section className="import-preview"><div className="panel-head"><div><h3>{busy?"מייבא תנועות":"תצוגה מקדימה"}</h3><p>{pending.filename} · {pending.rows.length} רשומות</p></div><button disabled={busy} aria-label="ביטול הייבוא" onClick={()=>setPending(null)}><X/></button></div>{busy&&progress?<div className="rounded-xl border border-emerald-100 bg-emerald-50/60 p-4" aria-live="polite"><div className="mb-2 flex items-center justify-between gap-3"><b className="text-sm text-emerald-950">{Math.round(progress.processed/progress.total*100)}% הושלמו</b><span className="text-xs text-slate-600">{progress.processed} מתוך {progress.total}</span></div><Progress value={progress.processed/progress.total*100}/><div className="mt-3 flex gap-4 text-xs text-slate-600"><span>{progress.imported} נקלטו</span><span>{progress.skipped} דולגו</span></div><small className="mt-2 block text-xs text-slate-500">הייבוא מתקדם בפועל. אין לסגור את החלון עד לסיום.</small></div>:<><div className="import-preview-list">{pending.rows.slice(0,5).map((row,index)=><article key={index}><CheckCircle2/><div><b>{String(row.donorName||"ללא שם — יועבר לבדיקה")}</b><small>{String(row.movementType)==="expense"?"הוצאה":row.donorId||row.idNumber||row.phone||row.email?"הכנסה · כולל זיהוי":"הכנסה · זיהוי לפי שם"} · {String(row.amount)} {String(row.currency)} · {String(row.date)}</small></div></article>)}</div>{pending.rows.length>5&&<small>ועוד {pending.rows.length-5} רשומות</small>}</>}<Button disabled={busy} onClick={()=>void confirmImport()}>{busy?`מייבא… ${progress?Math.round(progress.processed/progress.total*100):0}%`:"אישור וייבוא הנתונים"}</Button></section>}<div className="import-tip"><b>חשוב:</b> הסכום הוא שדה החובה היחיד. תאריך חסר יקבל את תאריך הייבוא, ושם חסר ייקלט כ״ללא שם״ ויועבר לבדיקת התאמה. ת״ז, טלפון או דוא״ל מסייעים למנוע כפילויות.</div></DialogContent></Dialog></>
}
