...
function ImportHistory(){
  ...
  return <section className="config-panel import-history"><div className="panel-head"><div><h2>היסטוריית ייבוא תנועות</h2><p>מעקב וביטול מרוכז של קבצים שיובאו למערכת.</p></div><FileSpreadsheet/></div>{notice&&<div className="save-notice"><CheckCircle2/>{notice}</div>}{loading?<p className="m-0 px-4 py-5 text-sm text-slate-500">טוען את היסטוריית הייבוא…</p>:runs.length===0?<p className="m-0 px-4 py-5 text-sm text-slate-500">עדיין לא בוצעו ייבואי תנועות.</p>:<div className="flex flex-col border-t border-slate-200">{runs.map(run=><article key={run.id} className="grid grid-cols-1 gap-3 border-b border-slate-100 p-4 md:grid-cols-[minmax(0,1.4fr)_minmax(230px,1fr)_auto] md:items-center"><div className="min-w-0"><b className="block break-words text-[15px] text-slate-800">{run.filename}</b><small className="mt-1 block text-xs text-slate-500">{new Intl.DateTimeFormat("he-IL",{dateStyle:"short",timeStyle:"short"}).format(new Date(run.createdAt))} · {run.rowsImported+run.rowsSkipped<run.rowsTotal?<span className="font-bold text-amber-700">הייבוא נקטע או עדיין בעבודה</span>:<span className="font-bold text-emerald-700">הושלם</span>}</small></div><dl className="m-0 grid grid-cols-3 gap-2"><div className="rounded-lg bg-emerald-50/60 px-2 py-2 text-center"><dt className="text-xs text-slate-500">בקובץ</dt><dd className="m-0 mt-0.5 text-[15px] font-bold text-emerald-950">{run.rowsTotal}</dd></div><div className="rounded-lg bg-emerald-50/60 px-2 py-2 text-center"><dt className="text-xs text-slate-500">נקלטו</dt><dd className="m-0 mt-0.5 text-[15px] font-bold text-emerald-950">{run.rowsImported}</dd></div><div className="rounded-lg bg-emerald-50/60 px-2 py-2 text-center"><dt className="text-xs text-slate-500">דולגו</dt><dd className="m-0 mt-0.5 text-[15px] font-bold text-emerald-950">{run.rowsSkipped}</dd></div></dl><Button className="w-full text-red-700 md:w-auto" variant="outline" disabled={deleting===run.id} onClick={()=>void remove(run)}><Trash2/>{deleting===run.id?"מבטל…":"ביטול הייבוא"}</Button></article>)}</div>}<p className="settings-help px-4 py-3">ביטול מוחק רק תנועות שנקלטו מאותו קובץ. תנועות שנוספו ידנית או התקבלו ב־Webhook אינן מושפעות.</p></section>
}
